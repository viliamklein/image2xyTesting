...
int image2xy_files(const char* infn, const char* outfn,
    ...
    for (kk=1; kk <= nhdus; kk++) {
        const char* ttype[] = {"X","Y","FLUX","BACKGROUND", "LFLUX", "LBG"};
        const char* tform[] = {"E","E","E","E","E","E"};
        const char* tunit[] = {"pix","pix","unknown","unknown","unknown","unknown"};
        long* fpixel;
        int a;
        int w, h;
        int bitpix;
        ...
        if (bitpix == 8 && do_u8 && !downsample) {
            ...
            // u8 image.
            params->image_u8 = (unsigned char *) malloc(naxisn[0] * naxisn[1]);
            if (!params->image_u8) {
        ...
        if (params->Lorder)
            ...
            ncols = 4;
        fits_create_tbl(ofptr, BINARY_TBL, params->npeaks, ncols, (char **) ttype, (char **) tform,
                        (char **) tunit, "SOURCES", &status);
